...
// ---------------------------------------------------------
// المسار الأول: استخراج الأفلام (الذي قمنا بعمله سابقاً)
// ---------------------------------------------------------
app.get('/api/page', async (req, res) => {
    ...
    try {
        ...
        $('div.Small--Box:not(.Season)').each((index, element) => { // استبعاد المواسم من هنا
            const box = $(element);
            ...
            moviesList.push({
                id, title, url: movieUrl, image: imageUrl, genres: genre, quality, imdb: imdbRating
            });
        });
...
// ---------------------------------------------------------
// المسار الثاني (الجديد): استخراج المواسم
// ---------------------------------------------------------
app.get('/api/seasons', async (req, res) => {
    ...
    try {
        ...
        $('section.allseasonss div.Small--Box.Season').each((index, element) => {
            ...
            // استخراج الصورة مع دعم التحميل المتأخر
            const imgTag = box.find('div.Poster img');
            const imageUrl = imgTag.attr('data-src') || imgTag.attr('data-lazy-src') || imgTag.attr('src') || "";

            const id = seasonUrl ? crypto.createHash('md5').update(seasonUrl).digest('hex') : "";

            // استخدام نفس الهيكل وإبقاء الحقول غير الموجودة فارغة
            seasonsList.push({
                id: id,
                title: title,
                url: seasonUrl,
                image: imageUrl,
                genres: "",    // لا يوجد تصنيف في صندوق الموسم
                quality: "",   // لا يوجد جودة ظاهرة في صندوق الموسم
                imdb: ""       // لا يوجد تقييم في صندوق الموسم
            });
        });
